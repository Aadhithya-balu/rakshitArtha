...
function getConfig() {
    return {
        baseUrl: String(process.env.ZOMATO_API_BASE_URL || '').trim().replace(/\/$/, ''),
        path: String(process.env.ZOMATO_ACTIVITY_PATH || '/v1/partners/:platformUserId/activity').trim(),
        apiKey: String(process.env.ZOMATO_API_KEY || '').trim(),
        accessToken: String(process.env.ZOMATO_ACCESS_TOKEN || '').trim(),
        mode: String(process.env.ZOMATO_INTEGRATION_MODE || process.env.PLATFORM_INTEGRATION_MODE || 'mock').trim().toLowerCase(),
    };
}
...
function normalizeActivity(rawResponse, context = {}) {
    ...
    const activity = payload.activity || payload.summary || payload.partner || payload;
    ...
    const rideOrOrderCount = Math.max(0, Math.round(toNumber(
        payload.rideOrOrderCount ?? activity.orderCount ?? activity.deliveriesCompleted ?? activity.jobCount ?? activity.completedTrips,
        0
    )));
    ...
    return {
        ...
        location: {
            ...
            label: payload.location?.label || activity.location?.label || 'Zomato',
        },
...
async function fetchWorkerActivity({ platformUserId, auth = {} } = {}) {
    if (!platformUserId) {
        throw new Error('platformUserId is required for Zomato sync');
    }
